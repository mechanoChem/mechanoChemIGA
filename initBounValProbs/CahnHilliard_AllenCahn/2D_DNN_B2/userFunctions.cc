...
void residual(bool dV,
  ...
  DNN *freeEn = (DNN *)user.parameters;
  std::vector<T> features(2), y;
  ...
  f_cc_a = 0.01*ddy_dxdx[0][0][0];
  f_ceta_a = 0.01*ddy_dxdx[0][1][0]*2.*c.val(1);
  f_eta_a = 0.01*dy_dx[1][0]*2.*c.val(1);
  ...
  f_cc_b = 0.01*ddy_dxdx[0][0][0];
  f_ceta_b = 0.01*ddy_dxdx[0][1][0]*2.*c.val(3);
  f_eta_b = 0.01*dy_dx[1][0]*2.*c.val(3);
