...
int boundaryConditions(AppCtx& user, double scale){
  ...
  ierr = IGASetBoundaryValue(user.iga,0,1,0,(-1. + sqrt(1. + sqrt(8./3.)*user.matParam["Es"]))*scale1);CHKERRQ(ierr);
  ierr = IGASetBoundaryValue(user.iga,1,1,1,(-1. + sqrt(1. - sqrt(2./3.)*user.matParam["Es"]))*scale1);CHKERRQ(ierr);
  ...
  ierr = IGASetBoundaryValue(user.iga,0,1,3,dVal + (-1. + sqrt(1. + sqrt(8./3.)*user.matParam["Es"]))*scale1);CHKERRQ(ierr);  
