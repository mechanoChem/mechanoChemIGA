...
#Sacado
#mpicxx -c -fPIC  -Wall -Wwrite-strings -Wno-unused-variable -Wno-unused-value  -Wno-uninitialized -Wno-strict-aliasing -Wno-unknown-pragmas -O3 -I/$PETSC_DIR/$PETSC_ARCH/include -I/$PETSC_DIR/include -I$PETIGA_DIR/$PETSC_ARCH/include -I$PETIGA_DIR/include -I$TRILINOS_DIR/include/ $fileName.c 
...
time mpiexec -np 1 ./$fileName -file_prefix "test" -N 100 -ch_monitor -ts_monitor -snes_monitor -snes_converged_reason -log_summary -dt 0.01 -ts_max_snes_failures 200 -snes_max_it 200  -snes_type newtontr -ksp_converged_reason -ksp_type preonly -pc_type lu -pc_factor_mat_solver_package mumps
